...
let blockchainNetwork = 'polygon-mumbai'
let networkPath = {
    wss: '',
    https: '',
    protocol: 'https'
}
if (blockchainNetwork == 'polygon-mumbai') {
    networkPath.https = envProcess.parsed.INFURA_POLYGON_MUMBAI_HTTPS + envProcess.parsed.INFURA_PROJECT_ID;
} else if (blockchainNetwork == 'polygon-mainnet') {
    networkPath.https = envProcess.parsed.INFURA_POLYGON_MAINNET_HTTPS + envProcess.parsed.INFURA_PROJECT_ID;
} else if (blockchainNetwork == 'ropsten') {
    networkPath.wss = envProcess.parsed.INFURA_ROPSTEN_WSS + envProcess.parsed.INFURA_PROJECT_ID;
    networkPath.https = ''
} else if (blockchainNetwork == 'rinkeby') {
    networkPath.wss = envProcess.parsed.INFURA_RINKEBY_WSS + envProcess.parsed.INFURA_PROJECT_ID;
    networkPath.https = ''
}
...
const provider = new HDWalletProvider({
    ...
    providerOrUrl: networkPath.protocol == 'https' ? networkPath.https : networkPath.wss,
});
...
Web3EthContract.setProvider(networkPath.protocol == 'https' ? networkPath.https : networkPath.wss);
