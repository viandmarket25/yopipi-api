...
let buyerSearchProduct={
  ...
  searchProductFromText:async()=>{
    return new Promise(async(resolve) => {
      ...
      let total=0
      let queryStatement="SELECT DISTINCT * FROM products JOIN product_media ON product_media.PRODUCT_ID=products.ID WHERE `TITLE` LIKE ? OR `DESCRIPTION` LIKE ? GROUP BY products.ID ORDER BY products.ID ASC    "
      //SELECT * FROM `products` join product_media on product_media.PRODUCT_ID=products.ID;
